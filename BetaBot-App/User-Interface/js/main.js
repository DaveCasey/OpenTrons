...
var app = {
	...
	loadFile : function(name){
		this.backend.load(name,function(object){

			app.pages.ingredients.nameCounter = 0;
			app.ingredients = {};
			app.pages.ingredients.ingredientList.innerHTML = '';
			for(var n in object.ingredients){
				app.pages.ingredients.addIngredient(object.ingredients[n]);
			}

			app.protocol = [];
			app.pages.job.jobList.innerHTML = '';
			for(var i=0;i<object.protocol.length;i++){
				app.pages.job.addStep(object.protocol[i]);
			}

			app.pages.ingredients.pipetteDropdown.value = object.pipette;
		});
	},

	throwError : function(string){
		var errorDiv = document.createElement('div');
		errorDiv.className = 'abs text';
		errorDiv.style.width = app.width*.8+'px';
		errorDiv.style.height = app.width*.4+'px';
		errorDiv.style.textAlign = 'center';
		errorDiv.style.left = app.gutter+app.width*.1+'px';
		errorDiv.style.top = app.navOffset*3+'px';
		errorDiv.style.backgroundColor = app.colors.yellow;
		errorDiv.innerHTML = '<br />Error: <br /><br />'+string;
	...
	prepareJob : function(isSaving){
		...
		for(var i in this.ingredients){
			var tempIngredient = {};
			tempIngredient.name = this.ingredients[i].name;
			if(!isSaving && (this.ingredients[i].x===undefined || this.ingredients[i].y===undefined || this.ingredients[i].z===undefined)){
				app.throwError('ingredient '+this.ingredients[i].name+' must be set');
				failed = true;
				return false;
			}
			else{
		...
		if(theJob.ingredients.length===0){
			app.throwError('No ingredients created');
			return false;
		}

		if(app.protocol.length===0){
			app.throwError('No Protocol Steps created');
			return false;
		}
		...
		if(!failed || isSaving){
			for(var i=0;i<this.protocol.length;i++){
				...
				if(!isSaving && isNaN(timeVal) || isNaN(actionVal)){
					failed = true;
					app.throwError('Error on Job step '+(i+1));
					return false;
				}
				else{
				...
				if(!failed || isSaving){
					...
					tempStep.ingredient = app.ingredients[ingTag].name;
