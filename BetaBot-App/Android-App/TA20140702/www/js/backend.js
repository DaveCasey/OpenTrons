...
Backend.prototype.pause = function(){
	if(this.testGUI){
		//
	}
	else {
		...
		opentrons.pause(success,failure);
	}
}
...
Backend.prototype.resume = function(){
	if(this.testGUI){
		//
	}
	else {
		...
		opentrons.resume(success,failure);
	}
}
...
Backend.prototype.save = function(name,object){
	...
	for(var i in tempFile.ingredients){
		if(tempFile.ingredients[i].div) delete tempFile.ingredients[i].div;
	}
...
Backend.prototype.load = function(name){
